...
// File: contracts/SiberianLocker.sol

pragma solidity 0.6.12;


/**
 * @dev SiberianLocker contract locks the liquidity (LP tokens) which are added by the automatic liquidity acquisition
 * function in SiberianToken.
 *
 * The owner of SiberianLocker will be transferred to the timelock once the contract deployed.
 *
 * Q: Why don't we just burn the liquidity or lock the liquidity on other platforms?
 * A: If there is an upgrade of SiberianSwap AMM, we can migrate the liquidity to our new version exchange.
 *
 * Website: https://siberianswap.finance
 * Twitter: https://twitter.com/SiberianSwap
 */
contract SiberianLocker is Ownable {
    ...
    function unlock(IBEP20 _token, address _recipient) public onlyOwner {
        require(_recipient != address(0), "SiberianLocker::unlock: ZERO address.");
